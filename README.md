...
This API is the middle layer: the demo app never talks to PayFast directly, and this API never touches PayFast's signing/ITN logic directly either — both go through [`@ngelekanyo/payfast`](https://www.npmjs.com/package/@ngelekanyo/payfast).

```mermaid
flowchart LR
    App["payfast-sub-app\n(React)"] -- "@ngelekanyo/payfast/client" --> API["payfast-subscribe-api\n(this repo)"]
    API -- "@ngelekanyo/payfast/server" --> PayFast["PayFast"]
    PayFast -- "ITN webhook (/api/payfast/notify)" --> API
    API -- "service role key" --> Supabase[("Supabase")]
    App -- "reads plans/subscriptions\n(RLS-restricted writes)" --> Supabase
```

- **`payfast-sub-app`** calls this API's `/api/payfast/*` routes through the `@ngelekanyo/payfast/client` SDK, and reads subscription/plan data from Supabase directly (writes are RLS-restricted to creating `pending` rows — see that repo's migrations).
- **This API** wraps `@ngelekanyo/payfast/server`'s Express router, supplying the callbacks that persist subscription state to Supabase using the service role key (bypassing RLS, since only trusted server code should ever set a subscription to `active`/`paused`/`cancelled`).
- **PayFast** posts ITN callbacks back to this API's `/api/payfast/notify`, which `@ngelekanyo/payfast/server` verifies (signature, merchant ID, source IP) before this API's callback ever runs.

To test the full loop locally, run `payfast-sub-app` and this API side by side, and expose this API's `/api/payfast/notify` with a tunnel (e.g. `ngrok http 3000`) so PayFast's sandbox can actually reach it — `NOTIFY_URL` in `.env` needs to be the tunnel's public URL, not `localhost`.
