...
const getSubscriptionOwnerById = vi.fn();
...
describe("requireInitiatePaymentOwnership", () => {
  let res: Response;
  let next: NextFunction;
  ...
  async function loadMiddleware() {
    const mod = await import("./RequireSubscriptionOwnership");
    return mod.requireInitiatePaymentOwnership;
  }

  it("rejects with 401 when the Authorization header is missing", async () => {
    const requireInitiatePaymentOwnership = await loadMiddleware();
    const req = { headers: {}, body: { m_payment_id: "sub-1" } } as unknown as Request;

    await requireInitiatePaymentOwnership(req, res, next);

    expect(res.status).toHaveBeenCalledWith(401);
    expect(next).not.toHaveBeenCalled();
  });

  it("rejects with 400 when m_payment_id is missing from the body", async () => {
    getUser.mockResolvedValue({ data: { user: { id: "user-1" } }, error: null });
    const requireInitiatePaymentOwnership = await loadMiddleware();
    const req = {
      headers: { authorization: "Bearer good-token" },
  ...
  it("rejects with 404 when m_payment_id does not match any subscription", async () => {
    getUser.mockResolvedValue({ data: { user: { id: "user-1" } }, error: null });
    getSubscriptionOwnerById.mockResolvedValue(null);
    const requireInitiatePaymentOwnership = await loadMiddleware();
    const req = {
      headers: { authorization: "Bearer good-token" },
      body: { m_payment_id: "sub-1" },
    } as unknown as Request;
  ...
  it("rejects with 403 when the caller does not own the referenced subscription", async () => {
    getUser.mockResolvedValue({ data: { user: { id: "user-1" } }, error: null });
    getSubscriptionOwnerById.mockResolvedValue("user-2");
    const requireInitiatePaymentOwnership = await loadMiddleware();
    const req = {
      headers: { authorization: "Bearer good-token" },
      body: { m_payment_id: "sub-1" },
  ...
  it("calls next when the caller owns the referenced subscription", async () => {
    getUser.mockResolvedValue({ data: { user: { id: "user-1" } }, error: null });
    getSubscriptionOwnerById.mockResolvedValue("user-1");
    const requireInitiatePaymentOwnership = await loadMiddleware();
    const req = {
      headers: { authorization: "Bearer good-token" },
      body: { m_payment_id: "sub-1" },
